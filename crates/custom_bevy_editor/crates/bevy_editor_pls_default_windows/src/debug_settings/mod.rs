...
impl Default for DebugSettingsWindowState {
    fn default() -> Self {
        Self { pause_time: false, wireframes: false, highlight_selected: true, open_debugdump_status: None }
    }
}
...
impl EditorWindow for DebugSettingsWindow {
    ...
    fn ui(world: &mut bevy::prelude::World, mut cx: bevy_editor_pls_core::editor_window::EditorWindowContext, ui: &mut egui::Ui) {
...
fn debug_ui(world: &mut World, state: &mut DebugSettingsWindowState, ui: &mut egui::Ui, type_registry: &TypeRegistry) {
...
fn debug_ui_options(world: &mut World, state: &mut DebugSettingsWindowState, ui: &mut egui::Ui, type_registry: &TypeRegistry) {
    Grid::new("debug settings").show(ui, |ui| {
        ...
        if ui.add(egui::DragValue::new(&mut speed).clamp_range(0..=20).speed(0.1)).changed() {
            time.set_relative_speed_f64(speed);
        }
        ui.end_row();

        let wireframe_enabled =
            world.get_resource::<RenderAdapter>().map_or(false, |adapter| adapter.0.features().contains(WgpuFeatures::POLYGON_MODE_LINE));
        ...
        ui.scope(|ui| {
            ...
            if ui_for_value(&mut state.wireframes, ui, type_registry) {
                world.get_resource_or_insert_with(WireframeConfig::default).global = state.wireframes;
            }
        });
        ui.end_row();
...
fn debug_ui_debugdump(world: &mut World, state: &mut DebugSettingsWindowState, ui: &mut egui::Ui) {
    let open_dot = |dot: &Option<String>, path: &str| -> Result<(), DebugdumpError> {
        ...
        let rendered = match debugdump::execute_dot(dot, format) {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Err(DebugdumpError::DotNotFound),
    ...
    ui.vertical(|ui| {
        ...
        if ui.button("Open render extract schedule").clicked() {
            let schedule_graph = world.get_resource::<debugdump::DotGraphs>().unwrap();
            if let Err(e) = open_dot(&schedule_graph.render_extract_schedule, "schedule_render_extract") {
                state.open_debugdump_status = Some(e);
            }
        }
