use bevy::{
    input::mouse::{MouseScrollUnit, MouseMotion, MouseWheel},
    prelude::*,
...
/// Adapted from [bevy_panorbit_camera](https://github.com/Plonq/bevy_panorbit_camera)
#[derive(Component)]
...
fn pan_orbit_camera(
    ...
    mut orbit_cameras: Query<(&mut PanOrbitCamera, &mut Transform, &mut Projection, &Camera)>,
) {
    ...
    for (mut pan_orbit, mut transform, mut projection, camera) in orbit_cameras.iter_mut() {
        ...
        if rotation_move.length_squared() > 0.0 {
            ...
            if let Some(win_size) = Some(Vec2::new(window.width(), window.height())) {
                let delta_x = {
